...
function stampCircle(world: World, cx: number, cy: number, r: number, element: number) {
  const r2 = r * r
  for (let dy = -r; dy <= r; dy++) {
    for (let dx = -r; dx <= r; dx++) {
      ...
      if (world.inBounds(x, y)) world.set(x, y, element)
    }
  }
}

export function paintCircle(world: World, cx: number, cy: number, r: number, element: number, mirror = false) {
  stampCircle(world, cx, cy, r, element)
  if (mirror) stampCircle(world, world.width - 1 - cx, cy, r, element)
}
...
export function paintLine(
  ...
  for (let i = 0; i <= steps; i++) {
    const t = i / steps
    paintCircle(world, x0 + (x1 - x0) * t, y0 + (y1 - y0) * t, r, element, mirror)
  }
}
